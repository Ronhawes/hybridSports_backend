...
const AddPlayer = async (req, res, next) => {
  try {
    const { fullName, email, idNo, phoneNo, gender, country, physicalCondition, price} = req.body;

    // Check if all required fields are provided
    if (!fullName|| !email || !idNo|| !phoneNo || !gender|| !country|| !physicalCondition|| !price) {
    ...
    const player = await prisma.tickets.create({
      data: {
